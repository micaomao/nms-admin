...
use Micaomao\NmsAdmin\Services\AdminMenuService;
use Micaomao\NmsAdmin\Traits\IconifyPickerTrait;
use Micaomao\NmsAdmin\Controllers\AdminController;
use Micaomao\NmsAdmin\Support\CodeGenerator\Generator;
use Micaomao\NmsAdmin\Services\AdminCodeGeneratorService;
use Micaomao\NmsAdmin\Support\CodeGenerator\GenCodeClear;
...
class CodeGeneratorController extends AdminController
{
    ...
    public function getPropertyOptions(Request $request)
    {
        ...
        $className = '\\Micaomao\\NmsAdmin\\Renderers\\' . $request->c;
     ...
     * 字段表单
     *
     * @return \Micaomao\NmsAdmin\Renderers\Card
     */
    public function columnForm()
    {
        ...
        return amis()->Card()->body([
            amis()->Alert()
                ->body(__('admin.code_generators.column_warning') . " <a href='https://github.com/micaomao/nms-admin/issues/5' target='_blank'>" . __('admin.show') . "</a> ")
                ->level('warning')
                ->showCloseButton()
                ->showIcon(),
     ...
     * 预览代码 按钮
     *
     * @return \Micaomao\NmsAdmin\Renderers\DialogAction
     */
