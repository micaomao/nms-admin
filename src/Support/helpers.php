...
if (!function_exists('admin_url')) {
    ...
    {
        $prefix = $needPrefix ? '/' . \Micaomao\NmsAdmin\Admin::config('admin.route.prefix') : '';
...
if (!function_exists('admin_resource_full_path')) {
    function admin_resource_full_path($path, $server = null)
    {
        ...
        if (url()->isValidUrl($path) || mb_strpos($path, 'data:image') === 0) {
            ...
            $disk = \Micaomao\NmsAdmin\Admin::config('admin.upload.disk');
        ...
        $scheme = 'http:';
        if (\Micaomao\NmsAdmin\Admin::config('admin.https', false)) {
            $scheme = 'https:';
        }
...
if (!function_exists('amis')) {
     ...
     * @param $type
     *
     * @return \Micaomao\NmsAdmin\Renderers\Amis|\Micaomao\NmsAdmin\Renderers\Component
     */
    function amis($type = null)
    {
        if (filled($type)) {
            return \Micaomao\NmsAdmin\Renderers\Component::make()->setType($type);
        }

        return \Micaomao\NmsAdmin\Renderers\Amis::make();
    }
}

if (!function_exists('amisMake')) {
    /**
     * @return \Micaomao\NmsAdmin\Renderers\Amis
     * @deprecated
     */
    function amisMake()
    {
...
if (!function_exists('file_upload_handle')) {
    ...
    {
        $storage = \Illuminate\Support\Facades\Storage::disk(\Micaomao\NmsAdmin\Admin::config('admin.upload.disk'));
...
if (!function_exists('settings')) {
    function settings()
    {
        return \Micaomao\NmsAdmin\Services\AdminSettingService::make();
    }
}
...
if (!function_exists('admin_user')) {
    function admin_user()
    {
        return \Micaomao\NmsAdmin\Admin::user();
    }
}
...
if (!function_exists('admin_abort')) {
    ...
    {
        throw new \Micaomao\NmsAdmin\Exceptions\AdminException($message, $data, $doNotDisplayToast);
    }
...
if (!function_exists('admin_pages')) {
    ...
    {
        return \Micaomao\NmsAdmin\Services\AdminPageService::make()->get($sign);
    }
}
